...
if getattr(st.session_state, 'logged_in', False):
    ...
    if navigation_option == "Make Predictions":
        ...
        # Crime Type filter
        crime_types = st.multiselect("Select Crime Type(s)", df['primary_type'].unique())
        ...
        # Check if the button is clicked and all filters are selected
        if load_data_button and crime_types and start_time and end_time:
            # Apply filters
            filtered_data = df[
                (df['primary_type'].isin(crime_types)) &
